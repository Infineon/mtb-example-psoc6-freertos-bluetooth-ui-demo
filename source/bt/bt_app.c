...
void bt_task(void* param)
{
    ...
    for(;;)
    {
       ...
       /* Command has been received from queue */
        ...
        if(nofify_value == INDICATION_ON)
        {
            bt_app_send_indication();
        }
        ...
        if (app_gatt_db_ext_attr_tbl[i].handle == attr_handle)
        {
            ...
            if (validLen)
            {
                ...
                case HDLD_CAPSENSE_BUTTON_CLIENT_CHAR_CONFIG:
                    ...
                    {
                        capsense_data.buttoncount= 2u; /* No.of CapSense button */
                        capsense_data.buttonstatus1= 0u; /* Button status byte1 */
                        capsense_data.buttonstatus2= 0u; /* Button status byte2 */
                ...
                case HDLC_RGB_LED_VALUE:
                    if ( len != 4 )
                    {
                        return WICED_BT_GATT_INVALID_ATTR_LEN;
                    }
                    board_led_set_color((uint8_t*) app_rgb_led);
                ...
                case HDLD_HTS_TEMPERATURE_MEASUREMENT_CLIENT_CHAR_CONFIG:
                    ...
                    if(GATT_CLIENT_CONFIG_INDICATION ==
                            app_hts_temperature_measurement_client_char_config[0])
                    {
                        bt_app_send_indication();
                    }
                    break;
                                                    ...
                                                    uint16_t len_req)
{
    ...
    case HDLC_CAPSENSE_BUTTON_VALUE:
        ...
        app_capsense_button[0] = capsense_data.buttoncount;
        app_capsense_button[1] = capsense_data.buttonstatus1;
        app_capsense_button[2] = capsense_data.buttonstatus2;
        break;
...
/*******************************************************************************
* Function Name: bt_app_send_indication
********************************************************************************
* Summary: Sends GATT indication.
*
* Parameters:
*    None
*
...
void bt_app_send_indication(void)
{
    ...
    if((GATT_CLIENT_CONFIG_INDICATION == \
                            ...
                            && (0 != bt_connection_id))
     {
        ...
        status = wiced_bt_gatt_server_send_indication( bt_connection_id,
        ...
        if(WICED_BT_GATT_SUCCESS != status)
        {
            printf("Sending temperature sensor indication failed\r\n");
        }
